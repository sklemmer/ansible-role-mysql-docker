...
control "general" do
  ...
  describe docker_image('mysql:latest') do
    it { should exist }
...
control "plain" do
  impact 0.7
  title "Test mysql without open ports and shared socket"

  # container status
  describe docker_container('mysql-plain') do
    it { should exist }
    it { should be_running }
    its('image') { should eq 'mysql:latest' }
    its('repo') { should eq 'mysql' }
    its('tag') { should eq 'latest' }
...
control "with-port" do
  impact 0.7
  title "Test mysql with open ports"

  # container status
  describe docker_container('mysql-with-port') do
    it { should exist }
    it { should be_running }
    its('image') { should eq 'mysql:latest' }
    its('repo') { should eq 'mysql' }
...
#  impact 0.7
#  title "Test mysql with shared socket"
#
#  # container status
#  describe docker_container('mysql-with-socket') do
#    it { should exist }
#    it { should be_running }
#    its('image') { should eq 'mysql:latest' }
#    its('repo') { should eq 'mysql' }
...
#  impact 0.7
#  title "Test mysql with open ports and shared socket"
#
#  # container status
